        ...
        <div class="col-sm-6 col-xs-5">
            <div class="dataTables_length" id="example-datatable_length">
                <div class="btn-group btn-group-sm pull-left">
                    <a href="{{ route('create_country') }}" class="btn btn-primary">Create Country</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <table id="general-table" class="table table-striped table-vcenter">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ route('edit_country',$item->id) }}">{{ $item->title }}</a></td>
                        <td><a href="javascript:void(0)" class="label <?php echo $item->status == 1? 'label-success">ON': 'label-warning">OFF'; ?></a></td>
                        <td><a href="{{route('change_country', $item->id)}}" class="label <?php echo $item->menu == 1? 'label-success">ON': 'label-warning">OFF'; ?></a></td>
                        <td class="text-center">
                            <div class="btn-group btn-group-xs">
                                <a href="{{ route('edit_country',$item->id) }}" data-toggle="tooltip" title="Edit" class="btn btn-default"><i class="fa fa-pencil 2x"></i></a>
                                <a onclick="return confirm('Click OK to confirm delete.');" href="{{ route('delete_country', $item->id) }}" data-toggle="tooltip" title="Delete" class="btn btn-danger"><i class="fa fa-times"></i></a>
                            </div>
                        </td>
                    </tr>
                    @endforeach
