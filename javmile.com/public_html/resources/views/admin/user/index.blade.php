        ...
        <div class="col-sm-6 col-xs-5">
            <div class="dataTables_length" id="example-datatable_length">
                <div class="btn-group btn-group-sm pull-left">
                    <a href="{{ route('create_user') }}" class="btn btn-primary">Create user</a>
                </div>
            </div>
        </div>
        <form action="{{ route('admin_user') }}" method="get" enctype="multipart/form-data" class="form-horizontal">
        ...
        <table id="general-table" class="table table-striped table-vcenter">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ route('edit_user',$item->id) }}">{{ $item->username }}</a></td>
                        <td>{{ $item->email }}</td>
                        <td>{{ $item->group['title'] }}</td>
                        <td><a href="javascript:void(0)" class="label <?php echo $item->status == 1? 'label-success': 'label-warning'; ?>"><?php echo $item->status == 1? 'ON': 'OFF'; ?></a></td>
                        <td class="text-center">
                            <div class="btn-group btn-group-xs">
                                <a href="{{ route('edit_user',$item->id) }}" data-toggle="tooltip" title="Edit" class="btn btn-default"><i class="fa fa-pencil"></i></a>
                                <a href="{{ route('delete_user', $item->id) }}" data-toggle="tooltip" title="Xóa" class="btn btn-danger"><i class="fa fa-times"></i></a>
                            </div>
                        </td>
                    </tr>
                    @endforeach
