        ...
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <div class="col-md-6">
                <!-- Basic Form Elements Block -->
                    ...
                    <div class="form-group">
                        <label class="col-md-3 control-label" for="example-select">Parent category</label>
                        <div class="col-md-9">
                            <select id="example-select" name="parent_id" class="form-control" size="1">
                                <option value="">Select a category if that isn't parent category</option>
                                @foreach($categories as $key => $category)
                                <option value="{{ $category->id}}" <?php echo $category->id==$result->parent_id? 'selected': ''; ?>>{!! $category->title !!}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
