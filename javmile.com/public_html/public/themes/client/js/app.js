...
(function(d, s, id) {
    ...
    js.src = '//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.6&appId='+$('[property="fb:app_id"]').attr('content');
...
$(document).ready( function(){
    ...
    $('body').on('click', '.tab-switch', function(){
        var i = $(this);
        $('.tab-pane').addClass('hide');
        $('#'+i.find('a').attr('href')).removeClass('hide');
    });
    ...
    $('body').on('click', '.btn-bookmark', function(){
        var i = $(this);
        $.ajax({
            url: $('.user-tool').attr('bookmark-action'),
            type: 'POST',
            data: {key: $('.user-tool').attr('able'), _token: $('[name="csrf_token"]').attr('content') },
            dataType: 'json',
            success: function(res){
                if (res.add) {
                    i.html('<i class="gi gi-bookmark"></i> Bỏ quan tâm');
                }else{
                    i.html('<i class="gi gi-bookmark"></i> Quan tâm');
                }
            },
            error: function(err){
            },
    ...
    $('.alert-slide').height($('.main-slide').find('img').height());

    // $(window).scroll(function() {
    //     if ($(this).siblings().not('.sidebar-scrolled') && $(window).width() > 974) {
    //         if ($(this).scrollTop() >= ($('.content-group').offset().top - 54)) {
    //             $('.sidebar-scroll').addClass('sidebar-scrolled');
    //         }
    //     }
    //     if ($(this).scrollTop() < $('.content-group').offset().top) {
    //         $('.sidebar-scroll').removeClass('sidebar-scrolled');
    //     }
    // });
    if ($(window).width() > 751) {
        $('.film-tooltip').tooltipster({
